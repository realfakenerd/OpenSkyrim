...
- [ ] No linting or static analysis errors (using `cargo check`, `svelte-check`, `tsc`, and `luacheck`)
- [ ] Works correctly on mobile (specifically testing Android/Poco F5 compatibility where applicable)
- [ ] Documentation updated if needed (in `/docs` folder)
...
```bash
# Run all checks
cd src-tauri && cargo check && cd .. && bun svelte-check && bun x tsc && luacheck scripts/ && bun test
```
