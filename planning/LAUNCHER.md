...
> *"Skyrim game directory not automatically found. Please select your Skyrim Special Edition installation folder."*
...
* `modern_assets/scripts/` (Transpiled Lua 5.4 scripts)

---

### Step 3: Transformation Progress Bar (Library Call)
The launcher imports `openskyrim_converter` directly as a Rust crate dependency. It invokes the conversion functions in a background Rust thread while updating the GUI progress bar:

```rust
// Inside openskyrim_launcher
use openskyrim_converter::{NifConverter, TextureConverter, EsmConverter};
...
1. The launcher saves the path configuration to `openskyrim_config.json`.
2. Clicking **"PLAY OPENSKYRIM"** spawns the game engine binary (`openskyrim_engine`):
   ```rust
   use std::process::Command;

   pub fn launch_game_engine() {
       Command::new("./openskyrim_engine")
           .arg("--config")
           .arg("openskyrim_config.json")
           .spawn()
